...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'parse_pdf': {
                const { file_path, file_content, options = {} } = args;
            ...
            case 'parse_image_text': {
                const { image_path, image_content, options = {} } = args;
            ...
            case 'html_to_markdown': {
                const { html_content, html_file, options = {} } = args;
            ...
            case 'extract_tables': {
                const { file_path, content, content_type = 'auto', options = {} } = args;
            ...
            case 'summarize_document': {
                const { file_path, content, content_type = 'auto', summary_level = 'detailed', options = {} } = args;
...
// Helper functions
async function parsePdf(filePath, fileContent, options = {}) {
...
async function parseImageText(imagePath, imageContent, options = {}) {
    const startTime = Date.now();
    try {
        let buffer;
        if (imageContent) {
        ...
        // Perform OCR
        const worker = await (0, tesseract_js_1.createWorker)(options.language || 'eng');
...
async function htmlToMarkdown(htmlContent, htmlFile, options = {}) {
    const startTime = Date.now();
    try {
        let html;
        if (htmlContent) {
...
async function extractTables(filePath, content, contentType = 'auto', options = {}) {
    const startTime = Date.now();
    try {
        let text;
...
async function summarizeDocument(filePath, content, contentType = 'auto', summaryLevel = 'detailed', options = {}) {
    const startTime = Date.now();
    try {
        let text;
        let metadata = {};
...
// Utility functions
function extractSimpleTables(text, options = {}) {
    const tables = [];
...
function extractHtmlTables(html, options = {}) {
    const $ = cheerio.load(html);
    const tables = [];
...
function generateSummary(text, level, wordLimit, focusAreas) {
